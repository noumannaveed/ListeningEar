const Images = {
    logo: require('../assets/images/image21.png'),
    logo1: require('../assets/images/image31.png'),
    sound_wave: require('../assets/images/sound-wave-transparent.png'),
    profile: require('../assets/images/image4.png'),
    profile1: require('../assets/images/image5.png'),
    question: require('../assets/images/image51.png'),
    waiting: require('../assets/images/waiting-room.png'),
    wifi: require('../assets/images/wifi1.png'),
    Ellipse: require('../assets/images/Ellipse6.png'),
    sign: require('../assets/images/signboard1.png'),
    phone: require('../assets/images/phone-call.png'),
    mic: require('../assets/images/mic.png'),
    mic_off: require('../assets/images/mics.png'),
    mute: require('../assets/images/mute.png'),
    volume: require('../assets/images/volume.png'),


};
export { Images };